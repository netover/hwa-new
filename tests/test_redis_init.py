...
async def test_redis_initializer_basic():
    ...
    with patch('resync.core.redis_init.redis.Redis.from_url', return_value=mock_client):
        ...
        assert mock_client.set.call_count == 2, "Set should be called twice (lock and test key)"
        mock_client.get.assert_called_once()
        assert mock_client.delete.call_count == 2, "Delete should be called twice (lock and test key)"
