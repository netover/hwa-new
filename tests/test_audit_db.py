...
class TestAuditDB:
    ...
    def test_add_audit_record_success(self, sample_memory):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM audit_queue WHERE memory_id = ?", (sample_memory["id"],)
        )
        row = cursor.fetchone()
    ...
    def test_update_audit_status_success(self, sample_memory):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT status FROM audit_queue WHERE memory_id = ?", (sample_memory["id"],)
        )
        row = cursor.fetchone()
...
class TestAuditRecords:
    ...
    def test_concurrent_operations(self, temp_db):
        ...
        with patch("resync.core.audit_db.DATABASE_PATH", temp_db):
            pending = get_pending_audits()
            thread_records = [
                r for r in pending if r["memory_id"].startswith("thread_")
            ]
    ...
    def test_sql_injection_protection(self, temp_db):
        ...
        with patch("resync.core.audit_db.DATABASE_PATH", temp_db):
            ...
            malicious_record = next(
                (r for r in pending if "DROP TABLE" in r["memory_id"]), None
            )
