...
async def test_wal_recovery():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        cache1 = AsyncTTLCache(
            ttl_seconds=60,  # Long TTL for test
            enable_wal=True,
            wal_path=wal_path
        )
...
async def test_wal_delete_operations():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        set_entries = [e for e in all_entries if e.operation == WalOperationType.SET and e.key == "delete_test"]
        delete_entries = [e for e in all_entries if e.operation == WalOperationType.DELETE and e.key == "delete_test"]
