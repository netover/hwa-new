...
class TestAuditLogManager:
    ...
    def test_query_audit_logs_with_filters(self, audit_log_manager):
        ...
        audit_log_manager.log_audit_event("action2", "user2", {"data": "value2"}, severity="WARNING")
...
class TestConnectionPoolInitialization:
    ...
    async def test_database_connection_pool(self):
        ...
        config = ConnectionPoolConfig(
            pool_name="test_db_pool",
            min_size=1,
            max_size=5,
            timeout=10
        )
        db_pool = DatabaseConnectionPool(config, "sqlite:///file:memdb_test?mode=memory&cache=shared")
...
class TestAuditQueueIntegration:
    ...
    async def mock_audit_queue(self):
        ...
        queue.get_audit_metrics_sync = Mock(return_value={"total": 0, "pending": 0, "approved": 0, "rejected": 0})
    ...
    async def test_audit_api_logging(self, temp_db_path):
        ...
        with patch('resync.core.audit_log.get_audit_log_manager', return_value=db_manager):
            ...
            mock_audit_queue.get_audit_metrics_sync.return_value = {"total": 0, "pending": 0, "approved": 0, "rejected": 0}
            ...
            assert metrics_result == {"total": 0, "pending": 0, "approved": 0, "rejected": 0}
