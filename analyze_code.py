...
def analyze_project(root_dir: str = 'resync') -> Dict[str, Any]:
    ...
    if results['summary']['total_files'] > 0:
        total_coverage = sum(f.get('type_hint_coverage', 0) for f in results['files'] if 'error' not in f)
...
"""
    ...
    if results['summary']['files_without_type_hints']:
        ...
        for file_info in sorted(results['summary']['files_without_type_hints'], 
    ...
    for file_data in sorted(results['files'], key=lambda x: x.get('sloc', 0), reverse=True)[:10]:
        ...
        if file_data['high_complexity']:
            report += f"- **⚠️ Funções Complexas:** {len(file_data['high_complexity'])}\n"
    ...
    if results['summary']['avg_type_hint_coverage'] < 80:
        ...
        report += f"- Cobertura atual: {results['summary']['avg_type_hint_coverage']}%\n"
        report += "- Meta: > 90%\n"
