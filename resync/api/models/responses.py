...
class ProblemDetail(BaseModel):
    ...
    severity: Optional[str] = Field(
        None,
        description="Nível de severidade do erro",
        example="warning"
    )
...
class ValidationErrorDetail(BaseModel):
    ...
    value: Optional[Any] = Field(
        None,
        description="Valor fornecido (se aplicável)"
    )
...
def create_problem_detail(
    ...
    return ProblemDetail(
        ...
        errors=_format_details_as_errors(exception.details) if exception.details else None
    )
...
def create_paginated_response(
    ...
    """Cria resposta paginada com links HATEOAS.
    ...
    Args:
        ...
        page: Página atual
        page_size: Tamanho da página
        base_path: Caminho base para links (opcional)
        query_params: Parâmetros de query adicionais (opcional)
        
    Returns:
        PaginatedResponse configurado com links
    """
    total_pages = (total + page_size - 1) // page_size if page_size > 0 else 0
    
    # Criar links se base_path fornecido
    links = None
    if base_path:
        ...
        # Converter para dict
        links = {k: v.model_dump() for k, v in links.items()}
