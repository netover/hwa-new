"""
RAG Microservice File Ingestor

This module handles file uploads, saving, and processing for RAG operations.
"""
...
from resync.RAG.microservice.config import settings
    ...
    async def ingest_file(self, file_path: Path) -> bool:
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                # In production, this would use the CircuitBreakerAsyncKnowledgeGraph
                # For now, we'll just log
                logger.info("adding_chunk_to_knowledge_graph", chunk_index=i + 1)
                chunk_count += 1
            except Exception as e:
                logger.error(
                    "error_adding_chunk",
...
async def load_existing_rag_documents() -> int:
    """
            ...
            if file_path.is_file() and not file_path.name.startswith("."):
                ...
                if is_path_in_knowledge_base(file_path):
                    try:
                        ...
                        # In production, this would use the FileIngestor
                        # For now, we'll just log
                        processed_count += 1
...
def create_file_ingestor() -> FileIngestor:
    """
