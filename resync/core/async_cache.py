...
class AsyncTTLCache:
    ...
    def __init__(
        ...
        try:
            ...
            try:
                ...
                log_with_correlation(logging.DEBUG, "Loaded cache config from external module", correlation_id)
            except ImportError:
                # Handle the case where your_config_module is not available
                self.ttl_seconds = ttl_seconds
                self.cleanup_interval = cleanup_interval
                log_with_correlation(logging.WARNING,
                    "External config module not available, using defaults", correlation_id)
            ...
            runtime_metrics.record_health_check("async_cache", "initialized", {
                "ttl_seconds": self.ttl_seconds,
                "cleanup_interval": self.cleanup_interval,
                "num_shards": self.num_shards
            })
            log_with_correlation(logging.INFO, "AsyncTTLCache initialized successfully", correlation_id)

        except Exception as e:
            runtime_metrics.record_health_check("async_cache", "init_failed", {"error": str(e)})
            log_with_correlation(logging.CRITICAL, f"Failed to initialize AsyncTTLCache: {e}", correlation_id)
            raise
        finally:
    ...
    def _get_shard(self, key: str) -> Tuple[Dict[str, CacheEntry], asyncio.Lock]:
        ...
        try:
            ...
            if not (0 <= shard_index < self.num_shards):
                ...
                shard_index = 0
                logger.warning(f"Shard index bounds check failed for key hash {key_hash}, using fallback shard 0",
                             extra={"correlation_id": runtime_metrics.create_correlation_id({
                                 "component": "async_cache",
            ...
            shard_index = sum(ord(c) for c in str(key)[:10]) % self.num_shards  # Use first 10 chars
            logger.warning(f"Hash computation failed for key {repr(key)}: {e}, using fallback shard {shard_index}")
    ...
    async def _cleanup_expired_entries(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                log_with_correlation(logging.DEBUG, "AsyncTTLCache cleanup task cancelled", correlation_id)
                break
            except MemoryError as e:
                log_with_correlation(logging.ERROR, f"Memory error in AsyncTTLCache cleanup task: {e}", correlation_id)
                runtime_metrics.record_health_check("async_cache", "memory_error", {"error": str(e)})
                raise CacheError("Memory error during cache cleanup") from e
            except KeyError as e:
                log_with_correlation(logging.ERROR, f"Key error in AsyncTTLCache cleanup task: {e}", correlation_id)
                ...
                log_with_correlation(logging.ERROR, f"Runtime error in AsyncTTLCache cleanup task: {e}", correlation_id)
                runtime_metrics.record_health_check("async_cache", "runtime_error", {"error": str(e)})
                raise CacheError(f"Runtime error during cache cleanup: {e}") from e
            except Exception as e: # pragma: no cover
                log_with_correlation(logging.CRITICAL,
                    f"Unexpected error in AsyncTTLCache cleanup task: {e}", correlation_id, exc_info=True)
                runtime_metrics.record_health_check("async_cache", "critical_error", {"error": str(e)})
        ...
        for i in range(self.num_shards):
            ...
            async with lock:
                ...
                for key in expired_keys:
                    del shard[key]
                    log_with_correlation(logging.DEBUG, f"Removed expired cache entry: {key}", correlation_id)
                total_removed += len(expired_keys)

        if total_removed > 0:
            runtime_metrics.cache_evictions.increment(total_removed)
            log_with_correlation(logging.DEBUG, f"Cleaned up {total_removed} expired cache entries", correlation_id)
        ...
        try:
            ...
            async with lock:
                ...
                if entry:
                    ...
                    if current_time - entry.timestamp <= entry.ttl:
                        ...
                        if total_requests > 0:
                            ...
                            runtime_metrics.record_health_check("async_cache", "performance",
                                {"hit_rate": hit_rate, "total_requests": total_requests})
                        log_with_correlation(logging.DEBUG, f"Cache HIT for key: {repr(key)}", correlation_id)
                        return entry.data
                    else:
                        ...
                        if total_sets > 0:
                            ...
                            runtime_metrics.record_health_check("async_cache", "eviction_rate",
                                {"eviction_rate": eviction_rate, "total_evictions": total_evictions})
                        log_with_correlation(logging.DEBUG, f"Cache EXPIRED for key: {repr(key)}", correlation_id)
                ...
                if total_requests > 0:
                    ...
                    runtime_metrics.record_health_check("async_cache", "miss_rate",
                        {"miss_rate": miss_rate, "total_requests": total_requests})
                log_with_correlation(logging.DEBUG, f"Cache MISS for key: {repr(key)}", correlation_id)
                return None

        except (ValueError, TypeError) as e:
            log_with_correlation(logging.WARNING, f"Cache GET validation failed for key {repr(key)}: {e}", correlation_id)
            raise
        except Exception as e:
            log_with_correlation(logging.ERROR, f"Cache GET failed for key {repr(key)}: {e}", correlation_id)
            raise
        finally:
            runtime_metrics.close_correlation_id(correlation_id)
            ...
            async with lock:
                ...
                if not self._check_cache_bounds():
                    ...
                    for k in expired_keys[:10]:  # Remove up to 10 expired entries
                        del shard[k]
                        log_with_correlation(logging.DEBUG, f"Emergency cleanup removed expired key: {k}", correlation_id)

                    # If still over bounds, reject the operation
                    if not self._check_cache_bounds():
                        raise ValueError(f"Cache bounds exceeded: cannot add key {repr(key)} (cache too large)")

                shard[key] = entry
                runtime_metrics.cache_sets.increment()
                runtime_metrics.cache_size.set(self.size())
                log_with_correlation(logging.DEBUG, f"Cache SET for key: {repr(key)}", correlation_id)
        except Exception as e:
            log_with_correlation(logging.ERROR, f"Cache SET failed for key {repr(key)}: {e}", correlation_id)
            raise
        finally:
            runtime_metrics.close_correlation_id(correlation_id)

    def _validate_cache_inputs(self, key: Any, value: Any, ttl_seconds: Optional[int]) -> tuple[str, int]:
        """
        ...
        try:
            ...
            async with lock:
                if key in shard:
                    ...
                    runtime_metrics.cache_evictions.increment()
                    runtime_metrics.cache_size.set(self.size())
                    log_with_correlation(logging.DEBUG, f"Cache DELETE for key: {key}", correlation_id)
                    return True
                return False
        finally:
            runtime_metrics.close_correlation_id(correlation_id)
    ...
    async def rollback_transaction(self, operations: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            if len(operations) == 0:
                log_with_correlation(logging.DEBUG, "Empty operations list for rollback", correlation_id)
                return True
            ...
            for i, op in enumerate(operations):
                ...
                if op["operation"] not in ["set", "delete"]:
                    raise ValueError(f"Operation at index {i} has invalid operation: {op.get('operation')}")
                ...
                if not isinstance(key, (str, int, float, bool)):
                    try:
                        key = str(key)
                        if len(key) > 1000:
                            raise ValueError(f"Key too long in operation {i}: {len(key)} characters")
                    except Exception:
            ...
            for op in operations:
                try:
                    ...
                    log_with_correlation(logging.ERROR,
                        f"Failed to determine shard for key {repr(op['key'])}: {e}", correlation_id)
                    return False
            ...
            for shard_idx, ops in shard_operations.items():
                ...
                async with lock:
                    try:
                        ...
                        runtime_metrics.cache_size.set(self.size())
                        log_with_correlation(logging.DEBUG,
                            f"Rolled back {len(ops)} operations on shard {shard_idx}", correlation_id)

                    except Exception as e:
                        log_with_correlation(logging.ERROR,
                            f"Failed to rollback operations on shard {shard_idx}: {e}", correlation_id)
                        return False

            log_with_correlation(logging.INFO, f"Successfully rolled back {len(operations)} cache operations", correlation_id)
            return True

        except Exception as e:
            log_with_correlation(logging.ERROR, f"Cache rollback failed: {e}", correlation_id, exc_info=True)
            return False
        finally:
            runtime_metrics.close_correlation_id(correlation_id)
    ...
    def _check_cache_bounds(self) -> bool:
        ...
        if current_size > max_safe_size:
            logger.warning(f"Cache size {current_size} exceeds safe bounds {max_safe_size}",
                         extra={"correlation_id": runtime_metrics.create_correlation_id({
                             "component": "async_cache",
        ...
        if estimated_memory_mb > 100:  # Max 100MB per cache
            logger.warning(f"Estimated cache memory usage {estimated_memory_mb:.1f}MB exceeds 100MB limit",
                         extra={"correlation_id": runtime_metrics.create_correlation_id({
                             "component": "async_cache",
    ...
    def get_detailed_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "hit_rate": (runtime_metrics.cache_hits.value / total_requests) if total_requests > 0 else 0,
            "miss_rate": (runtime_metrics.cache_misses.value / total_requests) if total_requests > 0 else 0,
    ...
    def create_backup_snapshot(self) -> Dict[str, Any]:
        ...
        try:
            ...
            log_with_correlation(logging.INFO, f"Created cache backup snapshot with {snapshot['_metadata']['total_entries']} entries", correlation_id)
            return snapshot
    ...
    async def restore_from_snapshot(self, snapshot: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if "_metadata" not in snapshot:
                log_with_correlation(logging.ERROR, "Invalid snapshot format - missing metadata", correlation_id)
                return False
            ...
            if total_entries > 100000:  # Max 100K entries
                raise ValueError(f"Snapshot too large: {total_entries} entries (max 100000)")
            ...
            if snapshot_age > 3600:  # 1 hour max age
                log_with_correlation(logging.WARNING, f"Snapshot too old ({snapshot_age:.0f}s) - refusing restore", correlation_id)
                return False
            ...
            runtime_metrics.cache_size.set(self.size())
            log_with_correlation(logging.INFO, f"Restored {restored_count} entries from snapshot", correlation_id)
            return True

        except Exception as e:
            log_with_correlation(logging.ERROR, f"Failed to restore from snapshot: {e}", correlation_id, exc_info=True)
            return False
        finally:
            runtime_metrics.close_correlation_id(correlation_id)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_value = {"test": "data", "timestamp": time(), "correlation_id": correlation_id.id}
            ...
            log_with_correlation(logging.DEBUG, f"Cache health check PASSED - production ready: {result['production_ready']}", correlation_id)
            runtime_metrics.close_correlation_id(correlation_id)
            return result
