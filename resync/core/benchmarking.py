...
class PerformanceBenchmark:
    ...
    def compare_with_baseline(self, result: BenchmarkResult, baseline: BenchmarkResult) -> Dict[str, float]:
        """
        ...
        """
        comparison = {
            "avg_time_improvement": ((baseline.avg_time - result.avg_time) / baseline.avg_time) * 100,
            "p95_time_improvement": ((baseline.p95_time - result.p95_time) / baseline.p95_time) * 100,
            "p99_time_improvement": ((baseline.p99_time - result.p99_time) / baseline.p99_time) * 100,
            "error_rate_improvement": ((baseline.errors - result.errors) / baseline.iterations) * 100
        }
        return comparison
...
async def create_benchmark_runner(agent_manager: IAgentManager, tws_client: ITWSClient) -> SystemBenchmarkRunner:
