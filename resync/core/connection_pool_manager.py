...
class DatabaseConnectionPool(ConnectionPool[Engine]):
    ...
    async def _setup_pool(self) -> None:
        ...
        try:
            ...
            if self.database_url.startswith("sqlite"):
                # SQLite uses StaticPool for single connections or NullPool
    ...
    async def _create_connection(self) -> Engine:
        """Return the SQLAlchemy engine as the connection."""
        if not self._engine:
            raise DatabaseError("Database engine not initialized")
        return self._engine
        
    async def _validate_connection(self, connection: Engine) -> bool:
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            is_healthy = await self._validate_connection(self._engine)
            check_time = time.time() - start_time
            
            self.stats.last_health_check = datetime.now()
            self.stats.active_connections = self._engine.pool.checkedout()
            self.stats.idle_connections = self._engine.pool.checkedin()
            self.stats.total_connections = self._engine.pool.size()
    ...
    async def get_connection(self):
        ...
        try:
            ...
            # Get connection (SQLAlchemy handles pooling)
            engine = await self._create_connection()
            yield engine
