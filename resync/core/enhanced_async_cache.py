...
class KeyLock:
    ...
    async def _cleanup_old_locks(self) -> None:
        ...
        oldest_keys = sorted(
            available_locks, 
            key=lambda k: self.lock_access_times.get(k, 0)
        )
...
class CacheShard:
        ...
        async with key_lock:
            ...
            if entry:
                ...
                if current_time - entry.timestamp <= entry.ttl:
                    logger.debug("Cache HIT for key: %s in shard %d", key, self.shard_id)
                    return entry.data
                else:
                    # Entry expired, remove it
                    del self.data[key]
                    logger.debug("Cache EXPIRED for key: %s in shard %d", key, self.shard_id)
    ...
    async def remove_expired_entries(self) -> int:
        ...
        async with shard_lock:
            ...
            if expired_keys:
                logger.debug("Removed %d expired entries from shard %d", 
                             len(expired_keys), self.shard_id)
