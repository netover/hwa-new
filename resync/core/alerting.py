...
def _initialize_slo_alerts(alerting_system):
    ...
    if "slo_api_response_time" not in existing_names:
        alerting_system.rules.append(AlertRule(
...
class AlertingSystem:
    ...
    async def evaluate_rules(self, metrics: Dict[str, Any]) -> List[Alert]:
        ...
        for kpi_name, is_compliant in slo_compliance.items():
            if not is_compliant:
                ...
                rule = next((r for r in self.rules if r.metric_name == kpi_name and r.name.startswith("slo_")), None)
                if rule:
    ...
    def _find_active_alert(self, rule_name: str, metric_name: str) -> Optional[Alert]:
        ...
        for alert in self.active_alerts:
            if alert.rule_name == rule_name and alert.metric_name == metric_name and not alert.acknowledged:
                return alert
        return None
    
    def _create_alert(self, rule: AlertRule, value: float, timestamp: Optional[datetime] = None) -> Alert:
