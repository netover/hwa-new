...
class AuditLogEntry(Base):  # type: ignore
    """
    ...
    severity = Column(String(50), nullable=True, default='INFO')  # INFO, WARNING, ERROR, CRITICAL
...
class AuditLogManager:
    ...
    def log_audit_event(
        ...
        try:
            ...
            import json
            details_json = json.dumps(details, default=str)  # Convert non-serializable objects to strings
            ...
            logger.debug(f"Audit event logged: action={action}, user_id={user_id}, audit_id={audit_id}")
            return audit_id
        ...
        try:
            with self.get_session() as session:
                ...
                metrics = {
                    ...
                    "by_severity": {k: int(v) for k, v in severity_counts if k is not None},
