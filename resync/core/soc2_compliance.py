...
class SOC2Control:
    ...
    def is_compliant(self) -> bool:
        ...
        return self.status in [ControlStatus.TESTED, ControlStatus.AUDITED, ControlStatus.COMPLIANT]
...
class AvailabilityMetric:
    ...
    def availability_score(self) -> float:
        ...
        return max(0.0, 100.0 - (self.unplanned_downtime_hours / 8760 * 100))  # 8760 = hours in year
...
class ProcessingIntegrityCheck:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            self.checksum_before == self.checksum_after and
            len(self.validation_errors) == 0 and
            self.integrity_score >= 99.9
        )
...
class SOC2ComplianceManager:
    ...
    def _initialize_soc2_controls(self) -> None:
        ...
        controls_data = [
            ...
            # Availability Controls
            {
                ...
                "risk_level": "high",
                "automated_testing": True,
                "evidence_required": ["uptime_logs", "monitoring_alerts", "availability_reports"]
            },
            {
                "control_id": "AVL-002",
            ...
            # Processing Integrity Controls
            {
                ...
                "risk_level": "high",
                "automated_testing": True,
                "evidence_required": ["validation_logs", "integrity_checks", "error_reports"]
            },
            {
                "control_id": "INT-002",
                ...
                "risk_level": "high",
                "automated_testing": True,
                "evidence_required": ["transaction_logs", "audit_trails", "reconciliation_reports"]
            },
            ...
            # Confidentiality Controls
            {
                ...
                "risk_level": "medium",
                "automated_testing": False,
                "evidence_required": ["data_classification_policy", "classification_logs"]
            },
            {
                "control_id": "CONF-002",
                ...
                "risk_level": "high",
                "automated_testing": True,
                "evidence_required": ["access_logs", "encryption_logs", "handling_procedures"]
            },
            ...
            # Privacy Controls
            {
                ...
                "risk_level": "high",
                "automated_testing": False,
                "evidence_required": ["privacy_policy", "consent_logs", "user_communications"]
            },
            {
                "control_id": "PRIV-002",
                ...
                "risk_level": "high",
                "automated_testing": True,
                "evidence_required": ["erasure_logs", "portability_logs", "consent_management"]
            },
            ...
            # Change Management
            {
                ...
                "criteria": [SOC2TrustServiceCriteria.SECURITY, SOC2TrustServiceCriteria.AVAILABILITY],
                "risk_level": "high",
                "automated_testing": False,
                "evidence_required": ["change_requests", "approval_logs", "testing_records"]
            },
            ...
            # Incident Response
            {
                ...
                "risk_level": "high",
                "automated_testing": False,
                "evidence_required": ["incident_logs", "response_plans", "post_mortem_reports"]
            }
        ]
    ...
    def generate_compliance_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "period_start": time.time() - (90 * 24 * 3600),  # Last 90 days
        ...
        compliant_controls = sum(1 for c in self.controls.values() if c.is_compliant())
        ...
        for criterion, scores in criteria_compliance.items():
            ...
            report["criteria_scores"][criterion.value] = {
                "score": score,
        ...
        for criterion, weight in weights.items():
            if criterion in criteria_compliance:
                weighted_score += report["criteria_scores"][criterion.value]["score"] * weight
                total_weight += weight

        report["overall_compliance_score"] = weighted_score / max(0.01, total_weight)
        ...
        report["control_status"] = dict(status_counts)
        ...
        if self.availability_metrics:
            avg_availability = sum(m.availability_score for m in self.availability_metrics) / len(self.availability_metrics)
            total_downtime = sum(m.total_downtime_seconds for m in self.availability_metrics)

            report["availability_summary"] = {
                "average_availability": avg_availability,
                "total_downtime_seconds": total_downtime,
                "target_availability": self.config.target_availability_percentage,
                "meets_target": avg_availability >= self.config.target_availability_percentage
            }

        # Processing integrity summary
        if self.processing_checks:
            avg_integrity = sum(c.integrity_score for c in self.processing_checks) / len(self.processing_checks)
            failed_checks = sum(1 for c in self.processing_checks if not c.is_valid)

            report["processing_integrity_summary"] = {
                "average_integrity_score": avg_integrity,
                "failed_checks": failed_checks,
                "total_checks": len(self.processing_checks),
        ...
        for incident in self.confidentiality_incidents:
            incident_counts[incident.severity] += 1

        report["confidentiality_incidents"] = {
            "total_incidents": len(self.confidentiality_incidents),
            "by_severity": dict(incident_counts),
            "unresolved_incidents": sum(1 for i in self.confidentiality_incidents if not i.resolved)
        }
    ...
    def get_control_status(self, control_id: Optional[str] = None) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    async def _reporting_worker(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.info(f"Compliance report generated with score: {report['overall_compliance_score']:.2f}")
    ...
    def _generate_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        not_implemented = [cid for cid, c in self.controls.items()
                          if c.status == ControlStatus.NOT_IMPLEMENTED]
        if not_implemented:
            recommendations.append(f"Implement the following {len(not_implemented)} controls: {', '.join(not_implemented[:5])}")

        # Check availability
        avail_summary = report.get("availability_summary", {})
        if not avail_summary.get("meets_target", True):
            recommendations.append(f"Availability target not met. Current: {avail_summary.get('average_availability', 0):.2f}%, Target: {self.config.target_availability_percentage}%")
        ...
        if not integrity_summary.get("meets_target", True):
            recommendations.append("Processing integrity target not met. Review data validation processes.")

        # Check evidence collection
        evidence_summary = report.get("evidence_summary", {})
        if evidence_summary.get("total_valid_evidence", 0) < len(self.controls) * 2:
            recommendations.append("Insufficient evidence collected. Ensure all controls have supporting evidence.")
