...
def mock_dependencies():
    ...
    with patch("resync.core.ia_auditor.audit_lock", new_callable=AsyncMock) as mock_audit_lock, \
...
class TestIAAuditor:
    ...
    async def test_cleanup_locks_handles_exception(self, mock_dependencies):
        ...
        mock_dependencies["audit_lock"].cleanup_expired_locks.side_effect = Exception("Cleanup failed")
    ...
    async def test_analyze_memory_skips_if_invalid(self, mock_dependencies):
        ...
        with patch("resync.core.ia_auditor._validate_memory_for_analysis", new_callable=AsyncMock) as mock_validate:
