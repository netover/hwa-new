...
class PerformanceBenchmarkSuite:
    ...
    async def benchmark_async_cache_cleanup(self) -> None:
        ...
        for i in range(1000):
            await cache.set(f"key_{i}", f"value_{i}", ttl_seconds=0.01)  # Expire quickly
        ...
        print(f"Cache cleanup time: {cleanup_time:.4f} seconds for {cache.size()} entries")
    ...
    async def benchmark_tws_batch_operations(self) -> None:
        ...
        self.results['tws_improvement_factor'] = sequential_time / parallel_time if parallel_time > 0 else 0
    ...
    async def benchmark_agent_initialization(self) -> None:
        ...
        for i in range(20):
            config = AgentConfig(
                ...
                tools=["tws_status_tool"] if i % 2 == 0 else ["tws_troubleshooting_tool"],
        ...
        self.results['agent_improvement_factor'] = sequential_time / parallel_time if parallel_time > 0 else 0
    ...
    async def benchmark_websocket_concurrent_connections(self) -> None:
        ...
        self.results['websocket_connections_per_second'] = num_connections / total_time if total_time > 0 else 0
