...
class CacheBenchmark:
    ...
    async def run_concurrent_benchmark(
        ...
        latencies = [lat for worker_latencies in all_latencies for lat in worker_latencies]
    ...
    async def run_all_benchmarks(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            for operation in ['get', 'set', 'mixed']:
                self.results[f'original_{operation}'] = await self.run_single_operation_benchmark(
                    'AsyncTTLCache', original_cache, operation
                )
                self.results[f'enhanced_{operation}'] = await self.run_single_operation_benchmark(
                    'EnhancedAsyncTTLCache', enhanced_cache, operation
                )
                
            # Concurrent benchmarks
            for workers in [10, 50, 100]:
                self.results[f'original_concurrent_{workers}'] = await self.run_concurrent_benchmark(
                    'AsyncTTLCache', original_cache, num_workers=workers
                )
                self.results[f'enhanced_concurrent_{workers}'] = await self.run_concurrent_benchmark(
    ...
    def print_results(self) -> None:
        ...
        print("-" * 60)
        print(f"{'Operation':<10} | {'AsyncTTLCache':<20} | {'EnhancedAsyncTTLCache':<20} | {'Improvement':<10}")
        print("-" * 60)
        
        for op in ['get', 'set', 'mixed']:
            orig = self.results.get(f'original_{op}', {}).get('operations_per_second', 0)
            enhanced = self.results.get(f'enhanced_{op}', {}).get('operations_per_second', 0)
            improvement = ((enhanced / orig) - 1) * 100 if orig > 0 else 0
            
            print(f"{op:<10} | {orig:<20.2f} | {enhanced:<20.2f} | {improvement:>9.2f}%")
            
        # Concurrent benchmark results
        print("\nConcurrent Performance (ops/sec):")
        print("-" * 70)
        print(f"{'Workers':<10} | {'AsyncTTLCache':<20} | {'EnhancedAsyncTTLCache':<20} | {'Improvement':<10}")
        print("-" * 70)
        
        for workers in [10, 50, 100]:
            orig = self.results.get(f'original_concurrent_{workers}', {}).get('operations_per_second', 0)
            enhanced = self.results.get(f'enhanced_concurrent_{workers}', {}).get('operations_per_second', 0)
            improvement = ((enhanced / orig) - 1) * 100 if orig > 0 else 0
            
            print(f"{workers:<10} | {orig:<20.2f} | {enhanced:<20.2f} | {improvement:>9.2f}%")
            
        # Latency results for concurrent tests
        print("\nLatency Results (ms) for 100 Workers:")
        print("-" * 70)
        print(f"{'Metric':<10} | {'AsyncTTLCache':<20} | {'EnhancedAsyncTTLCache':<20} | {'Improvement':<10}")
        print("-" * 70)
        
        orig_results = self.results.get('original_concurrent_100', {})
        enhanced_results = self.results.get('enhanced_concurrent_100', {})
        
        for metric in ['avg_latency_ms', 'p95_latency_ms', 'p99_latency_ms', 'max_latency_ms']:
            orig = orig_results.get(metric, 0)
            enhanced = enhanced_results.get(metric, 0)
            improvement = ((orig / enhanced) - 1) * 100 if enhanced > 0 else 0  # Lower is better for latency
            
            metric_name = metric.replace('_latency_ms', '')
            print(f"{metric_name:<10} | {orig:<20.2f} | {enhanced:<20.2f} | {improvement:>9.2f}%")
            
        # Cleanup benchmark
        print("\nCleanup Performance:")
        print("-" * 70)
        print(f"{'Metric':<15} | {'AsyncTTLCache':<20} | {'EnhancedAsyncTTLCache':<20} | {'Improvement':<10}")
        print("-" * 70)
        ...
        duration_improvement = ((orig_duration / enhanced_duration) - 1) * 100 if enhanced_duration > 0 else 0
        
        orig_throughput = orig_cleanup.get('entries_per_second', 0)
        enhanced_throughput = enhanced_cleanup.get('entries_per_second', 0)
        throughput_improvement = ((enhanced_throughput / orig_throughput) - 1) * 100 if orig_throughput > 0 else 0
        
        print(f"{'Duration (s)':<15} | {orig_duration:<20.3f} | {enhanced_duration:<20.3f} | {duration_improvement:>9.2f}%")
        print(f"{'Entries/sec':<15} | {orig_throughput:<20.2f} | {enhanced_throughput:<20.2f} | {throughput_improvement:>9.2f}%")
