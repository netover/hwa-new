...
async def demonstrate_key_level_locking(cache: EnhancedAsyncTTLCache) -> None:
    ...
    print(f"Estimated time with shard-level locking: {sum(times1) + sum(times2):.2f} seconds")
    print(f"Speedup from key-level locking: {(sum(times1) + sum(times2))/total_time:.2f}x")
...
async def demonstrate_parallel_cleanup(cache: EnhancedAsyncTTLCache) -> None:
    ...
    duration = time.time() - start_time
    print(f"Parallel cleanup of {num_entries} entries completed in {duration:.3f} seconds")
