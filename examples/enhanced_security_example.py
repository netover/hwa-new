...
class SecureLoginRequest(BaseModel):
    ...
    client_fingerprint: Optional[str] = Field(
        None,
        description="Client fingerprint for device recognition"
    )
...
async def register_user(
    ...
    email_result = await validate_email(request.email, SecurityLevel.HIGH, security_validator)
    ...
    password_result = await validate_password(request.password, SecurityLevel.HIGH, security_validator)
...
async def validate_user_input(
    ...
    return {
        ...
        "threat_detected": result.threat_detected.value if result.threat_detected else None,
...
async def example_secure_operation(user_id: str, operation: str):
    ...
    async with validator.security_context(context) as security_ctx:
        ...
        # Log security event
        from resync.api.validation.enhanced_security import SecurityEventLog, SecurityEventType, SecurityEventSeverity
